...
/**
 * @description 接口返回数据格式化成MenuOptions
 * @param {User.Menus} menuList 菜单列表
 * @returns {Menu.MenuOptions[]}
 */
export function formatMenus(menuList: Menus.Response[]): Menu.MenuOptions[] {
  function loop(menuList: Menus.Response[]): Menu.MenuOptions[] {
    let newMenuList: Menu.MenuOptions[] = [];
    for (let i = 0; i < menuList.length; i++) {
      const menu: Menus.Response = menuList[i];
      let _menu: Menu.MenuOptions = {
        ...menu,
        children: menu.children ? loop(menu.children) : undefined,
        // path: menu.path,
        name: menu.code
        // meta: {
        //   icon: i === 0 ? "HomeFilled" : menu.icon,
        //   title: menu.name
        // }
      };
      newMenuList.push(_menu);
    }
    return newMenuList;
  }
