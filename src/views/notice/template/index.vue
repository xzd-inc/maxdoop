<template>
  <div class="table-box">
    <ProTable ref="proTable" :columns="columns" :request-api="getTemplateList" :init-param="initParam" :tool-button="['refresh']">
...
import { getTemplateList, addNoticeTemplate, editNoticeTemplate, deleteNoticeTemplate } from "@/api/modules/notice";
...
const columns = reactive<ColumnProps<Notice.NoticeTempate>[]>([
  {
    prop: "searchText",
    label: "搜索",
    isShow: false,
...
const deleteNotice = async (params: Notice.NoticeTempate) => {
  await useHandleData(deleteNoticeTemplate, [params.id], `删除【${params.templateName}】`);
  proTable.value?.getTableList();
};

const drawerRef = ref<InstanceType<typeof EditDrawer> | null>(null);
const openDrawer = (title: string, row?: Notice.NoticeTempate) => {
  const params = {
    title,
    row: { ...row },
    api: title === "新增" ? addNoticeTemplate : editNoticeTemplate,
